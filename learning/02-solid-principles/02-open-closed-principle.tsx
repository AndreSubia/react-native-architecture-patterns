...
export default function App() {
  const [current, setCurrent] = React.useState<"email" | "sms" | "push">(
    "email",
  );
  ...
  return (
    <View style={styles.screen}>
      ...
      {current === "email" && (
        <EmailNotification message="Welcome to the app!" />
      )}
      {current === "sms" && <SMSNotification message="Your code is 1234" />}
      {current === "push" && (
        <PushNotification message="You have a new message!" />
      )}

      <View style={styles.buttons}>
        <TouchableOpacity
          onPress={() => setCurrent("email")}
          style={styles.button}
        >
          <Text>Email</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setCurrent("sms")}
          style={styles.button}
        >
          <Text>SMS</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => setCurrent("push")}
          style={styles.button}
        >
          <Text>Push</Text>
        </TouchableOpacity>
